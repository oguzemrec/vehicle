    ...
    : em{_em}
{
  ...
  if (em)
    for (auto const &t : tracks)
      em->insertConsumption(t->getName(), 0, false);
}
...
void movement::move(unsigned int speed) const
{
  for (auto const &t : tracks)
  {
    t->setSpeed(speed);
...
trackDirection track::getTrackDirection() const
{
  return dir;
}
...
std::ostream &operator<<(std::ostream &os, const movement &mv)
{
  ...
  {
    os << "{ " << t->getName() << ": Speed:" << t->getSpeed() << " Running State:" << t->getRunningState() << " Track Direction:" << static_cast<int>(t->getTrackDirection()) << "}" << std::endl;
  }
