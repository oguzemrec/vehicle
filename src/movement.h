...
constexpr double PWM_RESOLUTION = 4095;
constexpr double TRACK_ENERGY_CON_RATE  =0.001;
...
{
public:
explicit movement(const std::shared_ptr<energyManagament> &_em);

void move(unsigned int id, unsigned int speed) const;  //for specific motor
void move(unsigned int speed) const;  //for all motors

void stop(unsigned int id) const; // for specific motor
void stop() const; //for all motors
